...
public class Tree {
	...
	public boolean exists(Node node) throws Exception {
		...
		for (Node n : nodeList) {
			if (node.getDataItem().equals(n.getDataItem())) {
				if (Settings.trace()) {
					System.out.println("equals(" + node.getDataItem() + ","
							+ n.getDataItem() + ")");
				}
				return true;
			}
	...
	public void replace(Node node1, Node node2) throws Exception {
		if (root != null) {
			if (root == node1) {
				...
				if (parentNode1 != null) {
					if (parentNode1.getLeftChild() != null
							&& parentNode1.getLeftChild() == node1) {
						parentNode1.setLeftChild(node2);
					} else if (parentNode1.getRightChild() != null
							&& parentNode1.getRightChild() == node1) {
	...
	public static void swap(Tree treeOne, Node nodeOne, Tree treeTwo,
			Node nodeTwo) throws Exception {
		Node parentNodeOne = nodeOne.getParent();
		Node parentNodeTwo = nodeTwo.getParent();

		if (parentNodeOne != null) {
			if (parentNodeOne.getLeftChild() != null
					&& parentNodeOne.getLeftChild() == nodeOne) {
				parentNodeOne.setLeftChild(nodeTwo);
			} else {
				if (parentNodeOne.getRightChild() != null
						&& parentNodeOne.getRightChild() == nodeOne)
		...
		if (parentNodeTwo != null) {
			if (parentNodeTwo.getLeftChild() != null
					&& parentNodeTwo.getLeftChild() == nodeTwo) {
				parentNodeTwo.setLeftChild(nodeOne);
			} else {
				if (parentNodeTwo.getRightChild() != null
						&& parentNodeTwo.getRightChild() == nodeTwo) {
