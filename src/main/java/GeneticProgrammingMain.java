...
public class GeneticProgrammingMain {
	...
	public void process() throws Exception {
		...
		while (!done(startTime, currentMaxFitnessTree)
				&& output.getGenerationCount() < Settings.getMaxGeneration()) {
			...
			ArrayList<GeneticProgrammingTree> nextGenPopulation = GeneticOperators
			...
			ArrayList<GeneticProgrammingTree> children = GeneticOperators
					.crossoverTrees(population);
	...
	public void initializeOutputData(OutputData output,
			ArrayList<GeneticProgrammingTree> initialPopulation,
			GeneticProgrammingTree currentMaxFitnessTree) throws Exception {
		output.incrementGenerationCount();
		output.addFittestTreeInGeneration(currentMaxFitnessTree);
	...
	private void performFitnesEvaluation(
			ArrayList<GeneticProgrammingTree> nextGenPopulation)
			throws Exception {
		for (GeneticProgrammingTree gpTree : nextGenPopulation) {
			double fitness = GeneticProgrammingTree.evaluateFitness(
					TrainingData.getTrainingData(), gpTree);
			gpTree.setFitness(fitness);
		}
	}

	private GeneticProgrammingTree getCurrentMaxFitnessTree(
			ArrayList<GeneticProgrammingTree> population) {
		GeneticProgrammingTree currentMaxFitnessTree = population.get(0);
		return currentMaxFitnessTree;
	}

	private boolean done(long startTime, GeneticProgrammingTree gpTree)
			throws Exception {
	...
	private boolean bestSolutionFound(
			GeneticProgrammingTree currentMaxFitnessTree) throws Exception {
		boolean found = false;
		if (currentMaxFitnessTree.getFitness() <= Settings
				.getFitnessThreshold()) {
			found = true;

			if (Settings.trace()) {
				System.out
						.println("[Trace:bestSolutionFound()] *** Best fit solution found! ***");
			}
		}

		if (Settings.trace()) {
			System.out.println("[Trace} currentMaxFitness: "
					+ currentMaxFitnessTree.getFitness());
			System.out.println("[Trace} fitnessThreshold : "
					+ Settings.getFitnessThreshold());
		}
	...
	private boolean executionTimeExceeded(long startTime) throws Exception {
		...
		if (Settings.trace()) {
			...
			System.out.println("[Trace] Max Duration: "
		...
		if (runDuration > Settings.maxExecutionTime()) {
			if (Settings.trace()) {
				...
				System.out.println("[Trace] Max Duration: "
						+ Settings.maxExecutionTime());
				System.out
						.println("[Trace:bestSolutionFound()] *** Max exection time exceeded! ***");
			}
	...
	private ArrayList<GeneticProgrammingTree> getInitialPopulation(
